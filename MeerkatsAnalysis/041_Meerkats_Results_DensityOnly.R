...
## 1.4. Exploring the relationship between density and season ----
# -----------------------------------------------------------

seas_dens = dplyr::distinct(data.meerkats, season, year, .keep_all = T)[, c("year", "season", "density")]

seas_dens_lm = lm(density ~ season, data = seas_dens)
summary(seas_dens_lm)

tiff(filename = "Density_Season.tiff", 
     width = 3, 
     height = 3, 
     units = "in", 
     bg = "white", 
...
ggplot(seas_dens, aes(x = as.factor(season), y = density)) +
  geom_violin(draw_quantiles = c(0.025, 0.5, 0.975), size = 0.2) +
  stat_summary(fun = mean,  colour = "black",  geom = "point",  
               shape = 20,  size = 3, show.legend = FALSE, position = position_dodge(width = 0.75)) +
  scale_x_discrete(labels = c("Dry", "Wet")) +
  xlab("Season") +
  ylab(expression("Pop. density (indiv./"*km^2*")")) +
  theme_bw() +
...
plot.stoch.lambda.densityOnly = ggplot(sim.results,  aes(x = vital.rate,  y = stoch.lambda,  fill = scenario)) +
  ...
  theme_bw() +
  theme(axis.title.x = element_text(size = 9, colour = "black", margin = margin(t = 4, r = 0, b = 0, l = 0)), 
        axis.title.y = element_text(size = 9, colour = "black", margin = margin(t = 4, r = 4, b = 0, l = 0)), 
...
plot.legend = ggplot(sim.results, aes(x = vital.rate, y = var.lambda, fill = scenario)) +
  geom_boxplot(ymin = agg.meerkats.var$var.lambda[, 1], ymax = agg.meerkats.var$var.lambda[, 3], outlier.size = 0.1, size = 0.1) +
  stat_summary(fun = mean, colour = "darkred", geom = "point", 
               shape = 17, size = 1, show.legend = TRUE, position = position_dodge(width = 0.75)) +
  labs(title = "", x = "Perturbed vital rate", y = "Variance of annual \u03BB") +
...
results.fig = cowplot::ggdraw() +
  draw_plot(plot.stoch.lambda.densityOnly, x = 0, y = 0.2, width = 0.5, height = 0.8) +
  draw_plot(plot.var.lambda.densityOnly, x = .5, y = 0.2, width = .5, height = 0.8) +
  draw_plot(get_legend(plot.legend), x = 0.45, y = 0, width = 0.1, height = 0.2) +
  draw_plot_label(label = c("(a)", "(b)"), 
                  size = 10, 
                  fontface = "plain",
                  x = c(0.02, 0.52), y = c(0.995, 0.995))


tiff(filename = "Meerkats_DensityOnly.tiff",
     width=7,
     height=4,
     units="in",
     bg="white",
...
stochLambda_densityOnly = cowplot::ggdraw() +
  draw_plot(plot.stoch.lambda.densityOnly, x = 0, y = 0.2, width = 1, height = 0.8) +
  draw_plot(get_legend(plot.legend), x = 0.45, y = 0, width = 0.1, height = 0.2)



tiff(filename = "Meerkats_StochLambda_DensityOnly.tiff",
     width=4,
     height=4,
     units="in",
     bg="white",
...
write.csv(sim.results, "SimResults_Meerkats_DensityOnly.csv", row.names = F)
