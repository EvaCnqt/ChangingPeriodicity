...
dewy.buildmat.stochastic.siteC <- function(density, year){
  ...
  # Juveniles (J) to Reproductive (SR/LR)
  TSF3stoch.mat.siteC["SR", "J"] = predict(transJ.HG, newdata = data.frame(density = density, time = year), type = "response") * (1 - predict(transJ.HG, newdata = data.frame(density = density, time = year), type = "response"))
  TSF3stoch.mat.siteC["LR", "J"] = predict(transJ.HG, newdata = data.frame(density = density, time = year), type = "response") * predict(transJ.HG, newdata = data.frame(density = density, time = year), type = "response")
